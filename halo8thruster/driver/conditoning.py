...
class Conditioning:
    ...
    def erase_conditioning_stats(self):
        ...
        if erase == "y":
            ...
            self._comms.read(IDX_THRUSTER_CMD, SUB_COND_CLEAR, bytearray(val))
            print("Erasing Conditioning Stats...")
            count = self._comms.read(index, subindex, "<B")
            step = self._comms.read(index, 0x1, "<I")
            step_status = self.read(index, 0x2, "<I")
            self._mr.log(LogType.SYS, count)
            r = int(count/3)
            for v in range(0, r-1):
                seq_stat_cond = self._comms.read(index, 0x2 + (v*3), "<I")
                elapsed_ms = self._comms.read(index, 0x3 + (v*3), "<I")
                monitor_err = self._comms.read(index, 0x4 + (v*3), "<I")
                seq_stat_cond = '0x' + hex(seq_stat_cond)[2:].zfill(8)
                monitor_err = '0x' + hex(monitor_err)[2:].zfill(8)
                self._mr.log(LogType.SYS, f"[{v}] seq_stat_cond-{seq_stat_cond}, elapsed_ms-{elapsed_ms}, monitor_err-{monitor_err}")
